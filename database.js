...
(function (global) {
    ...
    switch (type) {
      ...
      case "complaints":
        return {
          ...
          status: row.status,
          date: row.date,
          photosList: (row.photos_list || []).length
            ? row.photos_list
  ...
  function fileKind(file) {
    if (isVideoFile(file)) {
      throw new Error(
        `Файл «${file.name}» не приймається. Дозволені лише фото та PDF.`,
      );
    }
    const t = (file.type || "").toLowerCase();
    const n = (file.name || "").toLowerCase();
    if (
      t.startsWith("image/") ||
      /\.(jpe?g|png|gif|webp|heic|heif|bmp)$/i.test(n)
    )
      return "image";
    if (t === "application/pdf" || n.endsWith(".pdf")) return "pdf";
    throw new Error(
      `Файл «${file.name}» не підтримується. Дозволені лише фото та PDF.`,
    );
  }
  ...
  async function loadDB() {
    ...
    mockDB = {
      ...
      complaints: (complaints.data || []).map((r) =>
        mapRowToApp(r, "complaints"),
      ),
      complaintComments: (comments.data || []).map((r) =>
        mapRowToApp(r, "complaintComments"),
      ),
      polls: (polls.data || []).map((r) => mapRowToApp(r, "polls")),
      pollVotes: votes.data || [],
      sanitaryComments: (sanitary.data || []).map((r) =>
        mapRowToApp(r, "sanitaryComments"),
      ),
      sanitaryReplies: [],
  ...
  function getCommentsForComplaint(complaintId) {
    return (mockDB.complaintComments || []).filter(
  ...
  function canViewComplaintThread(complaint, telegramUser) {
    ...
    return (
      complaint.telegramUserId && complaint.telegramUserId === telegramUser.id
    );
  }
  ...
  function getUserVote(pollId, voterTgId) {
    ...
    const v = mockDB.pollVotes.find(
      (x) => x.poll_id === pollId && x.voter_tg_id === voterTgId,
    );
  ...
  async function listBotVisitors(adminPassword) {
    const sb = getClient();
    const { data, error } = await sb.rpc("admin_list_bot_visitors", {
  ...
  async function getBotPushEnabled(adminPassword) {
    const sb = getClient();
    const { data, error } = await sb.rpc("admin_get_bot_push_enabled", {
      p_password: adminPassword,
    });
    if (error) throw error;
    return !!data;
  ...
  async function getUsersDashboard(adminPassword) {
    const sb = getClient();
    const { data, error } = await sb.rpc("admin_get_users_dashboard", {
      p_password: adminPassword,
    });
    if (error) throw error;
    return (
  ...
  async function updateDutySanitary(
    adminPassword,
    dutyId,
    { status, comment, dutyTime },
  ) {
    const sb = getClient();
  ...
  async function uploadFilesFromInput(fileInput, folder = "uploads") {
    ...
    for (const file of files) {
      if (file.size > MAX_FILE_MB * 1024 * 1024) {
        throw new Error(
          `Файл ${file.name} завеликий (макс. ${MAX_FILE_MB} МБ)`,
        );
      }
  ...
  let pushTestModeTgId = null; // null = broadcast to all; set to admin TG ID for test mode
  ...
  async function notifyPush(title, message, options) {
    ...
    if (!url || !secret) {
      console.warn(
        "Push вимкнено: немає NOTIFY_API_URL або NOTIFY_SECRET у config.public.js",
      );
      return { ok: false, reason: "no_config" };
    }
    const opts = options || {};
    const isTestMode = !!pushTestModeTgId;
    const specificTarget = opts.targetTgId ? String(opts.targetTgId) : null;

    // Determine effective target:
    // 1. Test mode (admin only) — bypasses global off
    // 2. Direct target (e.g. complaint reply to author) — bypasses global off
    // 3. Broadcast — respects notificationsEnabledCache
    const effectiveTargetId = isTestMode
      ? String(pushTestModeTgId)
      : specificTarget || null;

    // Skip only if global is OFF and this is a plain broadcast (no specific target, no test mode)
    if (!notificationsEnabledCache && !isTestMode && !specificTarget) {
      console.warn("Push пропущено: сповіщення вимкнено (global off)");
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          title,
          message,
          target_tg_id: effectiveTargetId,
        }),
      });
      let data = {};
      try {
  ...
  async function addComplaintComment(
    ...
    await loadDB();
    const c =
      complaintForNotify || mockDB.complaints.find((x) => x.id === complaintId);
  ...
  async function insertSuggestion({ content, telegramUser }) {
    const sb = getClient();
    const id = "sug_" + Date.now();
    const tg = telegramUser || {};
    const { error } = await sb.rpc("insert_suggestion", {
      p_id: id,
      p_content: content,
      p_tg_id: String(tg.id || ""),
      p_tg_username: tg.username || "",
      p_tg_name: tg.name || "",
    });
    if (error) throw error;
    // Notify admin only (internal call)
    await notifyPush(
      "💡 Нова пропозиція до додатку",
      `Від: ${tg.name || tg.username || "Анонім"}\n\n${content.length > 400 ? content.slice(0, 400) + "…" : content}`,
    );
  }

  async function listSuggestions(adminPassword) {
    const sb = getClient();
    const { data, error } = await sb.rpc("admin_list_suggestions", {
      p_password: adminPassword,
    });
    if (error) throw error;
    return data || [];
  ...
  async function deleteSuggestion(adminPassword, id) {
    const sb = getClient();
    const { error } = await sb.rpc("admin_delete_suggestion", {
      p_password: adminPassword,
      p_id: id,
    });
    if (error) throw error;
  ...
  async function insertSanitaryComment({
    adminPassword,
    content,
    parentId,
    authorName,
    ...
    if (!parentId) {
      if (!adminPassword)
        throw new Error("Тільки адмін може додавати коментар");
  ...
  global.DormDatabase = {
    ...
    setPushTestMode: (tgId) => {
      pushTestModeTgId = tgId ? String(tgId) : null;
    },
    getPushTestMode: () => pushTestModeTgId,
    validateFileInput,
